...
def select_dataset_class(config):
    ...
    if config["model_str"] in ["CNN", "CNNLSTM", "DynamicCNN", "DynamicCNNLSTM"]:
    ...
    if config["model_str"] in ["MomonaNet", "DynamicMomonaNet", "ELEC573Net", "OriginalELEC573CNN", "CNNModel3layer"]:
        return GestureDataset  # This is 2D
    elif config["model_str"] in ["RNN", "HybridCNNLSTM", "CRNN", "CTRLNet"]:
...
class GestureDataset(Dataset):
    ...
    def __init__(self, features, labels, sl=1, ts=64, stride=None):
        # NOTE: SL, TS, Stride all NOT used, just a polymorphism relic... have to be passed in for other objects
...
class GestureDataset_3D(Dataset):
    def __init__(self, features, labels, num_channels=16, sl=64, ts=1):
        """
        Dataset for gesture data where each sample is 3D:
        [sequence_length, num_channels].

        Args:
        - features (numpy.ndarray): Shape (num_samples, sequence_length, num_channels)
        - labels (numpy.ndarray): Shape (num_samples,)
        """

        self.features = torch.tensor(np.array(features), dtype=torch.float32)  # [N, 64, 16]
        self.labels = torch.tensor(np.array(labels), dtype=torch.long)         # [N]

        if self.features.ndim != 3:
            self.features = self.features.reshape(-1, num_channels, sl)  # Hardcoded...

        # Sanity check
        assert self.features.ndim == 3, f"Expected features to be 3D, got shape {self.features.shape}"
        assert self.labels.ndim == 1, f"Expected labels to be 1D, got shape {self.labels.shape}"
...
class Windowing_GestureDataset_3D(Dataset):
...
class Windowing_GestureDataset_4D(Dataset):
    def __init__(self, features, labels, sl=1, ts=64, stride=None):
        """
        Dataset for gesture data with optional sliding window.
        Args:
