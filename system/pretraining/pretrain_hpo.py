...
Each trial:
  - Trains for a short budget (30 epochs or until early stop at patience=5)
...
# ─────────────────────────────────────────────────────────────────────────────
# User split (edit to match your user_split.json)
# ─────────────────────────────────────────────────────────────────────────────

# ─────────────────────────────────────────────────────────────────────────────
# User split — FIRST FOLD ONLY!!! I dont want to pretrain a ton of models for each fold at this stage
# ─────────────────────────────────────────────────────────────────────────────
DEFAULT_USER_SPLIT = {
    "train_PIDs":            ["P102","P114","P119","P005","P107","P126","P132","P112",
...
def objective_metacnnlstm(trial: optuna.Trial, tensor_dict_path: str, user_split: dict, device: str):
    ...
    Architecture: 1 Conv + 3 LSTM + linear head — keep this fixed to match Meta paper.
    """
    config = {
...
def objective_tst(trial: optuna.Trial, tensor_dict_path: str, user_split: dict, device: str):
    ...
    """
    # Sample d_model and n_heads jointly to ensure divisibility
    d_model_choice = trial.suggest_categorical("d_model", [32, 64, 128])
    # n_heads options must divide d_model
...
def get_best_config(model_type: str, study: optuna.Study) -> dict:
    """
    Build a full config dict from the best HPO trial for a given model.
    Returns config ready to pass to build_model() and get_pretrain_dataloaders().
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--storage",     type=str, default=None,   help="Optuna storage URL (sqlite or MySQL)")
