...
from system.MAML_MOE.maml import *
...
def build_model_from_trial(trial, base_config=None):
    ...
    config["maml_alpha_init_eval"] = trial.suggest_float("maml_alpha_init_eval", 0.0001, 0.1)
    # Eval
...
def objective(trial):
    ...
    for fold_idx in range(NUM_FOLDS):
        ...
        #
        #    val_metrics = meta_evaluate(model, user_val_dl, config, maml_adapt_and_eval)
        ...
        for batch in episodic_val_loader:
            ...
            val_metrics = maml_adapt_and_eval(model, config, support_set, query_set)
